...
## 0.0.10 fix3-repair 안내 (남은 alert() 제거, 신규 오류 코드 없음)

0.0.10-fix3-repair는 app-core.js에 남아 있던 나머지 `window.alert()`를 전역 안내 카드(`showToast()`) 또는 버튼 텍스트 피드백으로 교체했으며 오류 판정 로직과 오류 코드는 추가/변경하지 않았다. 기존 오류 코드와 기록 조건은 0.0.10/0.0.10-fix2/0.0.10-fix3과 동일하다.

## 0.0.10 fix3 안내 (확인 흐름/상세 팝업 보정, 신규 오류 코드 없음)

0.0.10-fix3은 문제 항목/개선내역 상세 팝업 추가, 검은 시스템 confirm()/일부 alert() 제거 및 공작소 스타일 확인 팝업 적용, 전체점검 정상 확인 카드 추가 등 확인 흐름과 UI만 보정했으며 오류 판정 로직과 오류 코드는 추가/변경하지 않았다. 기존 오류 코드와 기록 조건은 0.0.10/0.0.10-fix2와 동일하다.

## 0.0.10 fix2 안내 (UI 보정, 신규 오류 코드 없음)

0.0.10-fix2는 상태 표시 공통화, 품질검수 진행 표시, 모달/버튼 UI, Blogger 전송 content 정리 등 표시/UI 로직만 보정했으며 오류 판정 로직과 오류 코드는 추가/변경하지 않았다. "Gemini API 호출 실패"/"Gemini 응답 파싱 실패"/"ZIP 검증 실패" 등 기존 오류 코드와 기록 조건은 0.0.10과 동일하다. 화면에 표시되는 실패 사유 카드에는 아이콘(❌)이 추가되었지만, 오류백과에 저장되는 메시지 문자열 자체는 바뀌지 않았다.

## 0.0.10 안내 (흐름 재정리, 오류 코드는 동일)

0.0.10은 업로드→패키지 점검→Gemini 품질검수 흐름과 화면 표시를 재정리했지만, 오류 이력에 기록되는 오류 코드("ZIP 파일 인식 실패", "ZIP 검증 실패", "Gemini API 호출 실패", "Gemini 응답 파싱 실패" 등)와 기록 조건은 이전 버전과 동일하다. 다만 "ZIP 검증 실패"의 의미가 조정되었다: 이제는 SEO 미통과가 아니라 필수 파일/이미지 구조 누락(패키지 점검 "실패")일 때만 기록된다. SEO 관련 항목(메타설명 없음, FAQ 없음 등)은 오류가 아니라 패키지 점검의 "주의 항목"으로 화면에 안내되며 오류 이력에는 남지 않는다.

## 0.0.9 안내 (실기 잔여 보정, 오류 코드는 동일)

0.0.9는 Gemini 품질검수 실패 시 화면에 보여주는 사유를 세분화했지만, 오류 이력에 기록되는 오류 코드/메시지("Gemini API 호출 실패"/"Gemini 응답 파싱 실패")와 기록 조건은 0.0.7/0.0.8과 동일하다. 화면에는 추가로 실패 사유 분류(Worker 연결 실패/인증 토큰 만료/AI_API_KEY 문제/Gemini 쿼터·사용량 초과/모델명 문제/응답 JSON 파싱 실패/네트워크 오류/알 수 없는 오류)와 실제 호출 주소를 함께 보여준다. 이 분류는 오류 메시지 문자열을 기준으로 한 화면 표시용 추정이며, 오류백과에 저장되는 항목 자체를 늘리지 않는다.

## 0.0.8 안내 (GUI 보정, 신규 오류 코드 없음)

0.0.8은 GUI/CSS/표시 위치만 보정했으며 오류 판정 로직과 오류 코드는 변경하지 않았다. "Gemini API 호출 실패"/"Gemini 응답 파싱 실패"는 0.0.7과 동일하게 gemini-review-module에서 기록되며, 표시 위치만 별도 팝업에서 자료실 상세 내부 카드로 바뀌었다(오류 이력 자체의 기록 방식은 변경 없음).
