 ...
 *
 * v1.2: v1.1 검수사항 반영 - convertTablesToCards가 새로 만드는 카드 안에서
 * 제목(h3)/본문(p)에 직접 넣던 inline font-size를 제거했다(margin/line-height만
 * 유지). 이미지 R2 업로드/URL 교체/runSaveFlow 등 나머지 핵심 흐름은 v1.1과 동일.
 */
  ...
  function convertTablesToCards(html) {
    ...
    tables.forEach((table) => {
      ...
      bodyRows.forEach((row) => {
        ...
        title.setAttribute("style", "margin:0 0 8px;");
        while (cells[0].firstChild) title.appendChild(cells[0].firstChild);
        card.appendChild(title);

        for (let i = 1; i < cells.length; i++) {
          const p = doc.createElement("p");
          p.setAttribute("style", "margin:4px 0;line-height:1.6;");
